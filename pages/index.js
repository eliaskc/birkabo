    ...
    <div className='container grid grid-cols-1 gap-y-8 mb-16 px-8 justify-items-center lg:gap-8 lg:gap-y-16 lg:grid-cols-4 lg:mb-16 lg:p-0 lg:py-8 lg:grid-rows-[2.5fr,1fr]'>
      ...
      {links.map((link) => (
        <Link key={link.href} href={link.href} className='w-full'>
          <button type='button' className='bg-button/0 border-4 border-black shadow-lg text-xl break-all px-4 font-bold w-full h-28 rounded-2xl lg:border-8 lg:h-40 lg:row-start-2 lg:hover:underline'>{link.label}</button>
        </Link>
      ))}
